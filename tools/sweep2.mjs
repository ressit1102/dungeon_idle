#!/usr/bin/env node
// tools/sweep2.mjs
// Sweep hero attack multiplier and enemy boss multiplier to find balance point.
...
function makeEnemy(type, level = 1, bossMultiplier = 2.2, isBoss = false) {
  ...
  const bossMult = isBoss ? bossMultiplier : 1;
...
async function runSimulations({ sims = 300, heroLevel = 1, opts = {} } = {}) {
  ...
  for (let i = 0; i < sims; i++) {
    ...
    const win = runOneFight(heroLevel, type, enemyLevel, opts);
    if (win) wins++;
  }
...
(async () => {
  ...
  const heroLevel = 1;
  const heroAttackGrid = [1.0, 1.2, 1.4, 1.6, 2.0];
  const bossMultGrid = [1.0, 1.4, 1.8, 2.2, 2.6];

  let results = [];
  console.log(`Sweeping heroAttackMult x bossMult (${heroAttackGrid.length}x${bossMultGrid.length}) ...`);
  for (const ham of heroAttackGrid) {
    for (const bm of bossMultGrid) {
      const res = await runSimulations({ sims: simsPerCombo, heroLevel, opts: { heroAttackMult: ham, bossMultiplier: bm } });
      const pct = res.winRate * 100;
      results.push({ ham, bm, pct });
      process.stdout.write(`heroMul=${ham} bossMul=${bm} => ${pct.toFixed(2)}%\n`);
    }
  }
  results.sort((a,b) => Math.abs(b.pct - 50) - Math.abs(a.pct - 50));
  // show closest to 50
  results.sort((a,b) => Math.abs(a.pct - 50) - Math.abs(b.pct - 50));
  console.log('\nTop candidates:');
  results.slice(0,6).forEach(r => console.log(`${r.pct.toFixed(2)}% — heroMul=${r.ham}, bossMult=${r.bm}`));
})();
